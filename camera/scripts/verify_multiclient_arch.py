...
checks = {
    ...
    "reserved viewer slots": "reserveClientSlot()" in text and "releaseClientSlot()" in text and "clientCountMux" in text,
    "requeue failure drops client": "xQueueSend(streamingClients, &client, 0) == pdPASS" in text and "releaseClientSlot();" in text,
    ...
    "stream handler never captures": "static void handleStream()" in text and "esp_camera_fb_get" not in text[text.index("static void handleStream()"):text.index("static void sendLightJson()")],
    ...
    "broadcast copies frame before network I/O": "memcpy(broadcastBuffer, activeFrame, size);" in text,
    "no task suspend wake race": "vTaskSuspend(nullptr)" not in text and "eTaskGetState(" not in text,
    "PSRAM required for broadcaster": "if (!psramFound())" in text and "PSRAM is required for multi-client streaming" in text and "CAMERA_FB_IN_PSRAM" in text,
    "supports existing camera controls": 'server.on("/light"' in text and 'server.on("/status"' in text,
}

# Streaming and diagnostic paths must never capture outside camera_capture_task.
streaming_section = text[text.index("static void handleStream()"):text.index("static void sendLightJson()")]
broadcast_section = text[text.index("static void stream_broadcast_task"):text.index("static void connectWiFi()")]
checks["only capture task reads camera"] = (
    "esp_camera_fb_get" not in streaming_section and
    "esp_camera_fb_get" not in broadcast_section and
    text.count("camera_fb_t *fb = esp_camera_fb_get();") == 1 and
