...
ESP32 比例 PWM 溫控系統，用於脂尾袋鼩（fat-tailed dunnart）蟄眠實驗。3 顆 DS18B20 分角色監控，Web 儀表板 + OLED 即時顯示 + CSV 資料匯出。
...
```
src/main.cpp  (單一 Arduino sketch，約 620 行)
...
│
├── 控制層
│   ├── readSensor() — 非阻塞轉換狀態機（750ms）
│   ├── controlTemp() — 雙層安全 + 比例 PWM
│   ├── setTecPwm() — 0~1.0 歸一化功率 → 0~255 PWM
│   └── emergencyStop() — 不可逆斷電
│
├── 網路層
...
1. **`server.handleClient()`** — 非阻塞 HTTP
2. **DS18B20 重掃描** — 每 10 秒嘗試（僅當 `!dsOk`）
3. **`readSensor()`** — 每 2 秒觸發一次非阻塞讀取
...
#### 目標溫度設定
- 製冷目標（10–35°C）：拉條 ⊕ 數字輸入框 ⊕ 滾輪
- 加熱目標（15–40°C）：同上
- 提示行：「巢穴 > X°C 需製冷 | 巢穴 < Y°C 需加熱」
...
巢穴 < safeMin（5°C）或 > safeMax（35°C）？
  ├── YES → 關 TEC，風扇全速（過熱）/ 60（過冷）
  └── NO ↓

巢穴 > coolTarget + 0.3°C？
  ├── YES → 製冷 mode=power = clamp(偏差/5.0, 0.15, 1.0)
  │         風扇 = 100 + 155×power（0-255）
  └── NO ↓

巢穴 < heatTarget - 0.3°C？
  ├── YES → 加熱 mode:power = clamp(偏差/5.0, 0.15, 1.0)
  │         風扇 = 80 + 175×power
  └── NO → 維持：關 TEC，風扇低速 40
```
...
```
偏差 = |巢穴溫度 - 目標溫度|
PWM 比例 = clamp(偏差 / 5.0, 0.15, 1.0)
PWM 值 = PWM 比例 × 255
```

- **Kp = 0.2 per °C**：每偏離目標 1°C，功率增加 20%
- **最小功率 15%**：防止 TEC 在極低功率下效率為零
- **死區 0.3°C**：防止製冷↔加熱往返切換
...
WiFi AP 模式，Port 80，IP `192.168.4.1`。所有端點使用 HTTP GET。
...
| `coolTarget` | float | 製冷目標溫度 °C |
| `heatTarget` | float | 加熱目標溫度 °C |
...
| `coolTarget` | 10–35 | 製冷目標 °C |
| `heatTarget` | 15–40 | 加熱目標 °C |
...
```
GET /control?system=1&coolTarget=18.5&heatTarget=28.0
```
...
| 緩衝區 | 上限 | 用途 |
|--------|------|------|
| `H` | 120 筆 | 圖表顯示（滑動窗口，只保留最近 4 分鐘） |
| `allData` | **無上限** | CSV 匯出（從開機到匯出當下的全部數據） |
